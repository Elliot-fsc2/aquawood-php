...
export default function FoodOrdering() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!mounted) return;
      if (error) {
        addToast("error", "Could not load orders", error.message);
        return;
      }
      setOrders(
        (data ?? []).map((r: any) => ({
  ...
  const addToCart = (item: MenuItem) => {
    setCart((c) => {
      ...
      if (exists)
        return c.map((i) => (i.menuItemId === item.id ? { ...i, quantity: i.quantity + 1 } : i));
  ...
  const updateQty = (id: string, delta: number) => {
    setCart((c) =>
      c.map((i) => (i.menuItemId === id ? { ...i, quantity: Math.max(1, i.quantity + delta) } : i)),
    );
  };
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-brand-700 mt-1">
            Point of sale, in-room dining, kitchen display, and menu management.
          </p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-brand-100 p-2 flex flex-wrap gap-1">
        ...
        {tab === "kitchen" && (
          <button
            onClick={() => {
              const activeKds = orders.filter(
                (o) => o.status === "Pending" || o.status === "Preparing" || o.status === "Ready",
              );
              const rows = activeKds
                .map(
                  (o) => `
              ...
              if (w) {
                ...
                w.document.body.style.cssText =
                  "margin:0;padding:16px;font-family:system-ui,sans-serif;background:#0f172a;color:#fff;";
                ...
                >
                  ...
                  <div className="p-3">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <div className="font-medium text-sm text-brand-900 leading-tight">
                        {item.name}
                      </div>
                      <div className="font-serif text-base text-brand-900 flex-shrink-0">
                        ₱{item.price}
                      </div>
                    </div>
                    <div className="text-[11px] text-brand-600 line-clamp-2">
                      {item.description}
                    </div>
                  </div>
                </button>
              ...
              <div className="grid grid-cols-2 gap-2">
                <label className="text-xs text-brand-700">
                  ...
                  <select
                    ...
                    className="mt-1 w-full px-2 py-1.5 border border-brand-200 rounded text-sm bg-white"
                  >
                ...
                <label className="text-xs text-brand-700">
                  ...
                  <input
                    ...
                    className="mt-1 w-full px-2 py-1.5 border border-brand-200 rounded text-sm"
                  />
                </label>
              </div>
              ...
              {cart.length === 0 ? (
                ...
                <div className="space-y-2">
                  {cart.map((item) => (
                    <div
                      key={item.menuItemId}
                      className="flex items-center gap-2 py-2 border-b border-brand-50 last:border-0"
                    >
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="w-6 h-6 rounded bg-brand-100 hover:bg-brand-200 flex items-center justify-center"
                        >
                        ...
                        <button
                          ...
                          className="w-6 h-6 rounded hover:bg-red-50 flex items-center justify-center text-red-600 ml-1"
                        >
            ...
            {cart.length > 0 && (
              <>
                <textarea
                  ...
                  rows={2}
                  className="w-full mt-3 px-2 py-1.5 text-xs border border-brand-200 rounded resize-none"
                />
                ...
                <div className="grid grid-cols-2 gap-2 mt-4">
                  <button
                    ...
                    className="py-2 bg-emerald-600 text-white rounded-md text-sm hover:bg-emerald-700 flex items-center justify-center gap-1"
                  >
                    <DollarSign className="w-4 h-4" /> Cash
                  </button>
                  <button
                    onClick={() => placeOrder("Card")}
                    className="py-2 bg-blue-600 text-white rounded-md text-sm hover:bg-blue-700"
                  >
                    💳 Card
                  </button>
                  {orderType === "Room Service" && (
                    <button
                      onClick={() => placeOrder("Room Charge")}
                      className="py-2 bg-brand-800 text-cream-50 rounded-md text-sm hover:bg-brand-900 col-span-2 flex items-center justify-center gap-1"
                    >
                      <BedDouble className="w-4 h-4" /> Charge to Room
                    </button>
                  )}
                  <button
                    onClick={() => placeOrder("Mobile")}
                    className={`py-2 bg-purple-600 text-white rounded-md text-sm hover:bg-purple-700 ${orderType !== "Room Service" ? "col-span-2" : ""}`}
                  >
                    📱 Mobile Pay
                  </button>
                </div>
      ...
      {receiptOrder && (
        <div
          className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 print:bg-white print:p-0"
          onClick={() => setReceiptOrder(null)}
        >
          <div
            className="bg-white rounded-xl max-w-md w-full max-h-[90vh] overflow-y-auto print:rounded-none print:max-w-full"
          ...
          >
            <div className="p-6 border-b border-brand-100 flex items-start justify-between print:hidden">
              ...
              <button
                onClick={() => setReceiptOrder(null)}
                className="text-brand-600 hover:text-brand-900 text-2xl leading-none"
              >
                ×
              </button>
            </div>
            <div className="p-6" id="order-receipt-print">
              <div className="text-center mb-4">
                ...
                <div className="text-xs text-brand-500">
                  {receiptOrder.type} · {receiptOrder.tableOrRoom} · {receiptOrder.createdAt}
                </div>
              </div>
              <div className="text-sm border-t border-b border-brand-100 py-3 mb-3">
                <div className="flex justify-between">
                  <span className="text-brand-600">Guest</span>
                  <span className="font-medium">{receiptOrder.guestName}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-brand-600">Status</span>
                  <span>{receiptOrder.status}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-brand-600">Payment</span>
                  <span>{receiptOrder.payment}</span>
                </div>
              </div>
              ...
              <div className="mt-3 pt-3 border-t border-brand-100 text-sm space-y-1">
                ...
                <div className="flex justify-between">
                  ...
                  <span>₱{receiptOrder.service.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-base pt-2 border-t border-brand-100 mt-2">
                  <span className="font-medium">Total</span>
                  <span className="font-serif text-xl">₱{receiptOrder.total.toFixed(2)}</span>
                </div>
              </div>
              {receiptOrder.notes && (
                <div className="mt-3 text-xs italic text-brand-600 bg-brand-50 p-2 rounded">
                  {receiptOrder.notes}
                </div>
              )}
            ...
            <div className="p-4 border-t border-brand-100 flex gap-2 print:hidden">
              <button
                onClick={() => {
                  window.print();
                  addToast("info", "Printing receipt", receiptOrder.id);
                }}
                className="flex-1 py-2.5 bg-brand-800 text-cream-50 rounded-md text-sm hover:bg-brand-900 flex items-center justify-center gap-1"
              >
                <Printer className="w-4 h-4" /> Print Receipt
              </button>
              <button
                onClick={() => setReceiptOrder(null)}
                className="flex-1 py-2.5 border border-brand-200 rounded-md text-sm hover:bg-brand-50"
              >
                Close
              </button>
            </div>
    ...
    <div className="bg-white rounded-xl border border-brand-100 p-6">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
        <div>
          ...
          <div className="text-xs text-brand-600">
            Track every order across POS, room service, and dine-in
          </div>
        </div>
        <div className="flex gap-2 items-center">
          {isAdmin && selectedIds.length > 0 && (
            <button
              onClick={bulkDelete}
              className="px-3 py-2 bg-red-600 text-white rounded-md text-xs hover:bg-red-700 flex items-center gap-1"
            >
          ...
          <select
            ...
            className="px-3 py-2 text-xs border border-brand-200 rounded-md bg-white"
          >
          ...
          <tbody className="divide-y divide-brand-50">
            {filtered.length === 0 && (
              <tr>
                <td
                  ...
                  className="py-10 text-center text-brand-500 text-sm"
                >
            ...
            {filtered.map((order) => (
              <tr key={order.id} className="hover:bg-brand-50/40">
                ...
                <td className="py-3">
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full ${order.type === "Room Service" ? "bg-purple-100 text-purple-800" : order.type === "Dine-In" ? "bg-blue-100 text-blue-800" : "bg-slate-100 text-slate-700"}`}
                  >
                ...
                <td className="py-3 text-xs">
                  <span
                    className={`px-2 py-0.5 rounded-full ${order.payment === "Unpaid" ? "bg-red-100 text-red-800" : "bg-emerald-100 text-emerald-800"}`}
                  >
                    {order.payment}
                  </span>
                </td>
      ...
      {buckets.map((bucket) => {
        ...
        return (
          <div key={bucket.status} className="bg-white rounded-xl border border-brand-100">
            <div
              className={`${bucket.color} text-white px-4 py-3 rounded-t-xl flex items-center justify-between`}
            >
              ...
              {bucketOrders.length === 0 ? (
                ...
                bucketOrders.map((order) => (
                  <div key={order.id} className="border border-brand-100 rounded-lg p-3">
                    ...
                    {order.notes && (
                      <div className="text-xs bg-amber-50 text-amber-800 px-2 py-1 rounded mb-3">
                        ⚠️ {order.notes}
                      </div>
                    )}
                    <div className="flex gap-1">
                      {bucket.status === "Pending" && (
                        <button
                          ...
                          className="flex-1 py-1.5 bg-amber-600 text-white rounded text-xs hover:bg-amber-700"
                        >
                          Start Preparing
                        </button>
                      )}
                      {bucket.status === "Preparing" && (
                        <button
                          ...
                          className="flex-1 py-1.5 bg-emerald-600 text-white rounded text-xs hover:bg-emerald-700"
                        >
                          Mark Ready
                        </button>
                      )}
                      {bucket.status === "Ready" && (
                        <button
                          ...
                          className="flex-1 py-1.5 bg-brand-800 text-cream-50 rounded text-xs hover:bg-brand-900"
                        >
                          Mark Served
                        </button>
                      )}
...
function MenuManagement({
  ...
  const save = async (data: Omit<MenuItem, "id"> & { id?: string }) => {
    ...
    if (data.id) {
      ...
      setMenu((m) =>
        m.map((item) => (item.id === data.id ? { ...item, ...data, id: data.id! } : item)),
      );
        ...
        <div className="flex gap-2 items-center">
          ...
          {isAdmin && (
            <button
              ...
              className="px-3 py-2 bg-brand-800 text-cream-50 rounded-md text-xs hover:bg-brand-900 flex items-center gap-1"
            >
          ...
          <tbody className="divide-y divide-brand-50">
            {filtered.map((item) => (
              <tr key={item.id} className="hover:bg-brand-50/40">
                ...
                <td className="py-3 text-brand-700 text-xs max-w-xs truncate">
                  {item.description}
                </td>
                ...
                <td className="py-3">
                  <button
                    onClick={() => isAdmin && toggleAvailable(item.id)}
                    className={`text-xs px-2 py-0.5 rounded-full ${item.available ? "bg-emerald-100 text-emerald-800" : "bg-red-100 text-red-800"} ${isAdmin ? "cursor-pointer hover:opacity-80" : "cursor-default"}`}
                  >
...
function MenuForm({
  ...
  return (
    <form
      ...
      className="mb-4 p-4 rounded-lg bg-brand-50 border border-brand-100"
    >
      <div className="text-xs uppercase tracking-wider text-brand-600 mb-3">
      ...
      <div className="grid md:grid-cols-3 gap-3">
        <label className="text-xs text-brand-700">
          ...
          <input
            ...
            className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-white text-sm"
          />
        </label>
        <label className="text-xs text-brand-700">
          Category
        ...
        <label className="text-xs text-brand-700 md:col-span-2">
          Description
          <input
            name="description"
            required
      ...
      <div className="flex justify-end gap-2 mt-3">
        <button
          type="button"
          onClick={onCancel}
          className="px-3 py-1.5 border border-brand-200 rounded-md text-xs hover:bg-white flex items-center gap-1"
        >
          <X className="w-3 h-3" /> Cancel
        </button>
        <button
          type="submit"
          className="px-3 py-1.5 bg-brand-800 text-cream-50 rounded-md text-xs hover:bg-brand-900 flex items-center gap-1"
        >
