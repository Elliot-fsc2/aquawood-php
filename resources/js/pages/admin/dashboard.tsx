...
import { BedDouble, BookOpen, Building2, CalendarCheck, Users } from 'lucide-react';
    ...
    const statCards = [
        {
            ...
            breakdown: [
                ...
                { label: 'Confirmed', value: stats.confirmed_bookings, color: 'text-green-600' },
                { label: 'Checked In', value: stats.checked_in, color: 'text-indigo-600' },
                { label: 'Checked Out', value: stats.checked_out, color: 'text-gray-600' },
        ...
        },
        {
            ...
            bg: 'bg-emerald-100',
            breakdown: [
                { label: 'Active', value: stats.active_guests, color: 'text-emerald-600' },
            ],
        },
        {
            title: 'Rooms',
            total: stats.available_rooms + stats.occupied_rooms + stats.maintenance_rooms + stats.booked_rooms,
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4">
                ...
                <div className="rounded-xl border bg-card p-6">
                    ...
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        <Link href={adminBookingsIndex()} className="flex items-center gap-3 rounded-lg border p-3 text-sm transition-colors hover:bg-muted">
                            <CalendarCheck className="h-5 w-5 text-blue-600" />
                            <span>Manage Reservations</span>
                        </Link>
                        <Link href={adminRoomsIndex()} className="flex items-center gap-3 rounded-lg border p-3 text-sm transition-colors hover:bg-muted">
                            <BedDouble className="h-5 w-5 text-amber-600" />
                            <span>Manage Rooms</span>
                        </Link>
                        <Link href={adminGuestsIndex()} className="flex items-center gap-3 rounded-lg border p-3 text-sm transition-colors hover:bg-muted">
